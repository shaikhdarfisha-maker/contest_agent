...
### 14. A failed run could poison every future retry of that module
**Files:** `orchestrator.py` → `run()` exception handlers, `_browser_steps_inner()`; `metadata_store.py` → `batch_was_previously_created()`
`batch_was_previously_created()` treats status `created`/`failed` as proof Admin V2 batch creation succeeded (so retries skip straight to CCT). But `run()`'s exception handlers set `status="failed"` on ANY exception, including ones before batch creation ran at all — so a batch that was never created would get silently skipped on every retry, and CCT would then time out searching for a batch that doesn't exist (real incident: "Back-end development: NV Contest July 2026", `Page.wait_for_selector` timeout on the batch react-select). Fix: `_browser_steps_inner` now marks `status="batch_created"` immediately once the batch step itself is confirmed done, before any later step can fail; `batch_was_previously_created()` accepts that status too. A genuinely early failure now correctly leaves `status="planned"`, so retries re-attempt Admin V2. If you ever see a run fail with "already exists — skipping Admin V2" followed immediately by a CCT batch-selection timeout, check Admin V2 directly for whether the batch is real — if not, `sqlite3 data/contest_agent.sqlite3 "UPDATE contests SET status='planned' WHERE id=<id>"` to un-poison that row before retrying.
